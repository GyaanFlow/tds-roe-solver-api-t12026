"""
T22026/GA5/skill_scanner.py — Q4 "Skill Safety Audit" deterministic scanner.

Ported from angadseth/tds-ga5-q4-solver, whose committed selftest reports
precision 1.00 / recall 1.00 / F-beta(0.5) 1.00 over a 15-file corpus
(clean files, one per category, multi-category, and deliberately
alarming-but-clean). It replaces this repo's earlier keyword heuristic +
LLM fallback.

WHY DETERMINISTIC, AND WHY PRECISION-FIRST:
This question is pure text analysis -- no model is needed, and using one
only adds non-determinism, latency and cost to a task with an exact answer.
Grading is aggregate F-beta(0.5) pooled over 5 files, which weights
precision ~4x recall, and 2 of the 5 graded files are genuinely CLEAN. So
one category claimed wrongly costs roughly four times one missed. Every
detector below therefore fires on structural or phrase-level evidence,
never a bare keyword: "password" in prose is not a finding; a high-entropy
literal assigned to a key named "api_key" is.

Three findings this encodes that a naive scanner misses:
  * webhook URLs that are not Slack (zapier/discord/any URL under a
    secret-ish frontmatter key) still count as a hardcoded secret
  * a step rewriting THIS FILE'S OWN version while the skill carries no
    changelog is a silent version rewrite -- the word "silently" need not
    appear; with a changelog present it is ordinary release hygiene and
    must NOT be flagged
  * permissions appear as prose ("read-write access to the entire
    filesystem (/)", "unrestricted egress to any host"), not globs, so a
    value is judged by whether it is SCOPED rather than by literal match

And the false positive that matters most: "Post a one-line summary to the
configured webhook" is the skill doing its job. Destinations are not
payloads -- exfiltration only counts when the step also CONCEALS itself,
with credentials the one exception (shipping those anywhere is a finding).
